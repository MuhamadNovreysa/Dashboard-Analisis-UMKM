...
export function formatCurrency(value: number): string {
  if (value >= 1000000) {
    return `Rp ${(value / 1000000).toFixed(2).replace(/\.?0+$/, "")} juta`
  } else if (value >= 1000) {
    return `Rp ${(value / 1000).toFixed(1).replace(/\.0$/, "")} ribu`
  }
...
function calculateCLVData(customerArray: any[]) {
  const ranges = [
    { range: "< 100 ribu", min: 0, max: 100000 },
    { range: "100 - 500 ribu", min: 100000, max: 500000 },
    { range: "500K - 1 juta", min: 500000, max: 1000000 },
    { range: "1 - 3 juta", min: 1000000, max: 3000000 },
    { range: "3 - 5 juta", min: 3000000, max: 5000000 },
    { range: "> 5 juta", min: 5000000, max: Number.POSITIVE_INFINITY },
  ]
